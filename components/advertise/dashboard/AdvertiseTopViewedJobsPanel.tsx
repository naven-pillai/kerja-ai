...
// Verbatim mirror of admin's TopViewedJobsPanel with two changes only:
// 1. Uses the public site's createSupabaseBrowserClient (same anon key
//    the admin's createSupabaseClient uses under the hood — identical
//    data access, calls the same top_jobs_by_event RPC).
// 2. Job title link points to public /jobs/[slug] rather than /jobs/[slug]/edit.
...
export default function AdvertiseTopViewedJobsPanel() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setError('Could not load top viewed jobs.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm border-l-4 border-l-blue-400 p-6 flex flex-col gap-4 h-full">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <TrendingUp className="w-4 h-4 text-blue-500" />
          <h3 className="text-sm font-semibold text-gray-800">Top Viewed Jobs</h3>
        </div>
            ...
            <li key={job.id} className="flex items-center justify-between gap-3 py-2.5">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="flex flex-col gap-0.5 min-w-0">
                  <Link
                    href={`/jobs/${job.slug}`}
                    className="text-sm font-medium text-gray-800 hover:text-blue-600 truncate block max-w-50"
                  >
                    {job.title}
                  </Link>
                  {job.companies?.name && (
              ...
              <div className="flex items-center gap-1 text-gray-500 shrink-0">
                ...
                <span className="text-xs font-semibold">{job.views.toLocaleString()}</span>
              </div>
            </li>
          ))}
        </ul>
